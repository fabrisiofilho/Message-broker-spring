...
import br.com.rabbitmq.servicetwo.message.constant.Exchange;
import br.com.rabbitmq.servicetwo.message.constant.Queues;
import br.com.rabbitmq.servicetwo.message.constant.RoutingKey;
...
public class RabbitMQConfig {
    ...
    public Queue queueFanout() {
        return new Queue(Queues.QUEUE_SERVICE_TWO_FANOUT, false);
    }

    @Bean
    public Queue queueTopic() {
        return new Queue(Queues.QUEUE_SERVICE_TWO_TOPIC, false);
    }

    @Bean
    public Queue queueDirect() {
        return new Queue(Queues.QUEUE_SERVICE_TWO_DIRECT, false);
    }
    ...
    public Binding bindingDirect(@Qualifier("queueDirect") Queue queue, DirectExchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with(RoutingKey.KEY_TWO);
    }
